 ...
 % temp_image_list;
if length(imageSet.layers) == 8
    frame_list{1} = imageSet.layers;
else
    frame_list = imageSet.layers;
end
img_size = [800 600];

for frame_num = 1:length(frame_list)
    image_list = frame_list{frame_num};
    for img_num = 1:length(image_list)
        glBindTexture(GL.TEXTURE_2D,texname_static((frame_num-1)*length(image_list)+img_num));
