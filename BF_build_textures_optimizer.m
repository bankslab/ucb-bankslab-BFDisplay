function texname_static = BF_build_textures_optimizer(trial_mode, exp_num)
...
elseif trial_mode == 1
    % what parameters are needed for each experiment?
    % hinge: vertex depth, vergence depth, hinge angle
    trial_params = strjoin(trial_parameters, '_');
    for fnum = 1:8
        fname = strcat('/BF_texture_files/optimizer/', exp_num, trial_params, num2str(fnum), '.hdr');
        image_list{fnum} = uint8(255 * hdrread(fname));
    end
end
