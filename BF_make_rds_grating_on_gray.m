...
function BF_make_rds_grating_on_gray(distance, numdots, grating_orientation, cyclesperdegree, diameter_size_degrees, arcmindisp, IPD, dotradius_arcmin, texname_static)
	...
	% translate & draw the background gray
	glTranslatef(0,0,-0.015);
	BF_bind_texture_to_square(texname_static,26);
	glTranslatef(0,0,0.015);
	% translate & draw the broadband cross
	cross_size_degree=0.6;
    if distance>0.5
        cross_offset=0.016;
    else
        cross_offset=0.005;
    end
	scalefactor=distance*tand(cross_size_degree);
    widthratio=15;
	glTranslatef(0,0,cross_offset);
	glScalef(scalefactor,scalefactor/widthratio,1);
	BF_bind_texture_to_square(texname_static,27);
	glScalef(1/scalefactor,widthratio/scalefactor,1);
	glScalef(scalefactor/widthratio,scalefactor,1);
	BF_bind_texture_to_square(texname_static,27);
	glScalef(widthratio/scalefactor,1/scalefactor,1);
	glTranslatef(0,0,-cross_offset);
	% end of background gray drawing
