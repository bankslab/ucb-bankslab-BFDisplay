...
experiment_type='fatigue_time_4'; % Experiment for measuring visual fatigue, staircase method
...
howmanyconditions=2; %number of experiment days
...
conditionrecordfilename = [pwd '/BF_data_files/Temporal_VA_conflict/condition_record/fatigue_time_4_additional_condition_record.txt'];
...
if (exist(conditionrecordfilename,'file')~=2) % if the record file does not exist, make one.
	...
	fprintf(file_record, '%d\t',6+conflict_order(1));
	condition_order(1)=6+conflict_order(1);
	fprintf(file_record, '%d\t',6+conflict_order(2));
	condition_order(2)=6+conflict_order(2);
	fprintf(file_record, '%d\n', finished_conditions);
% 	fclose(file_record);
end

% open record file
[t_initials,t_c1,t_c2,t_finished]=textread(conditionrecordfilename,'%s%d%d%d');
...
conflict_length=[1 5 1 25]; % index by conditions, unit is # of trials.
