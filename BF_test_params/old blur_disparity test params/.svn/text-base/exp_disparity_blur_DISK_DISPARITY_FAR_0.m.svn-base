...
scell{1} = set(s,...
    ...
    'pedestal',0.415,...            % Defined relative to viewer (m)
...
if (get(scell{1},'phase1_focusmult') == 0)
    depthoffset = FarMidDist - ShiftDiopters(get(scell{1},'stimDistance'),diopter_offset);
end
...
scell{2} = scell{1};
scell{2} = set(scell{2},'pedestal',0.395);
...
scell{3} = scell{2};
scell{3} = set(scell{3},'pedestal',0.375);
...
scell{3} = set(scell{3},'maxValue',1.0);
scell{3} = set(scell{3},'minValue',0);
