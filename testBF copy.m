...
currentSet = 1;
algorithm = 0; %0 blending, 1 optimized 
for j = 1:length(sets)
    for i = 1:4
        optimized{j}{i} = imresize(uint8(255*(hdrread([prefix sets{j} '/optimized_layers_' num2str(i) '.hdr'] ))), 0.5);
        blending{j}{i} = imresize(uint8(255*(hdrread([prefix sets{j} '/blending_layers_' num2str(i) '.hdr'] ))), 0.5);
    end   
end
...
function [] = setTextures()
    if algorithm == 0
        for i = 1:4
            tex(i) = Screen('MakeTexture',wid,blending{currentSet}{i}(:,:,:));
        end 
    elseif algorithm == 1
        for i = 1:4
            tex(i) = Screen('MakeTexture',wid,optimized{currentSet}{i}(:,:,:));
        end 
    end
end
